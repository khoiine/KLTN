...
const Collection = () => {
    ...
    const fetchSubCategories = async () => {
        try {
            console.log('🔍 Fetching subcategories from:', `${backendUrl}/api/subcategory/list`)
            const response = await axios.get(`${backendUrl}/api/subcategory/list`)
            console.log('📦 SubCategories API Response:', response.data)
            console.log('📊 SubCategories array:', response.data.subCategories)
            if (response.data.success) {
                setSubCategories(response.data.subCategories)
                console.log('✅ SubCategories state updated:', response.data.subCategories)
            }
        } catch (error) {
    ...
    // Auto-refetch every 30 seconds to get new categories/subcategories (optional)
    useEffect(() => {
