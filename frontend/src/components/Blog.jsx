    ...
    const fetchBlogs = async () => {
        try {
            ...
            if (response.data.success) {
                // Giới hạn 3 tin tức
                const limitedBlogs = response.data.blogs
                    .filter(blog => blog.featured) 
                    .slice(0, 3) 

                if (limitedBlogs.length < 3) {
                    const remaining = response.data.blogs
                        .filter(blog => !blog.featured)
                        .slice(0, 3 - limitedBlogs.length)
                    setBlogs([...limitedBlogs, ...remaining])
                } else {
    ...
    return (
        <div className='pt-8'>
            <div className='text-center text-3xl mb-10'>
                <Title text1={'TIN TỨC'} text2={'NỔI BẬT'} />
            </div>
